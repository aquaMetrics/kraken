...
result <- kraken(data, loess = TRUE, pass_fail = 0.60, n_try = 10)
```
...
result <- kraken(data, loess = FALSE, pass_fail = 0.60, n_try = 10)
distances <- result %>% filter(question == "breachDistanceBestFit")
distances$transect <- 1:nrow(distances)
knitr::kable(dplyr::select(distances, location_id, question, response) %>%
               mutate(location_id = gsub(pattern = "NA", replacement = "", .$location_id) ))
...
probs <- probability_non_linear(data$survey_data, loess = TRUE, n_try = 10)
overrides <- override(probs)
breachs <- breach(overrides)
areas <- kraken::area(breachs)
...
probs_1 <- suppressWarnings(
  probability_non_linear(pen_1$survey_data, loess = TRUE, n_try = 10))
breach_1 <- breach(probs_1)
area_1 <- kraken::area(breach_1)

pen_2 <- consecutive_stations(transects_pen_2)
probs_2 <- suppressWarnings(
  probability_non_linear(pen_2$survey_data, loess = TRUE, n_try = 10))
...
single_sf <- dplyr::bind_rows(sf::st_as_sf(area_2$ellipse),
                              sf::st_as_sf(area_1$ellipse))
...
paste(
  "Percentage diff: ",
  100 - round((100 / as.numeric(sf::st_area(areas$ellipse))) * as.numeric(areas$fifthPercentileArea$`5%`)), "%"
)
```
...
probs <- probability_non_linear(data$survey_data, n_try = 10)
overrides <- override(probs)
breachs <- breach(overrides)
areas <- kraken::area(breachs)
...
probs <- suppressWarnings(probability_non_linear(data$survey_data, n_try = 10))
overrides <- override(probs)
breachs <- breach(overrides)
areas <- kraken::area(breachs)
