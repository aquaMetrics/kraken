...
result <- kraken(data, loess = TRUE, good_moderate = 0.60, niter = 10)
```
...
result <- kraken(data, loess = FALSE, good_moderate = 0.60, niter = 10)
distances <- result %>% filter(question == "breachDistanceBestFit")
distances$transect <- 1:nrow(distances)
knitr::kable(dplyr::select(distances, location_id, question, response) %>% mutate(location_id = gsub(pattern = "NA", replacement = "", .$location_id) ))
...
probs <- probability_non_linear(data$survey_data, niter = 10)
overrides <- override(probs)
breachs <- breach(overrides)
areas <- kraken::area(breachs)
...
probs <- suppressWarnings(probability_non_linear(data$survey_data, niter = 10))
overrides <- override(probs)
breachs <- breach(overrides)
areas <- kraken::area(breachs)
...
probs <- probability_non_linear(data$survey_data, loess = TRUE, niter = 10)
overrides <- override(probs)
breachs <- breach(overrides)
