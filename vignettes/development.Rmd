...
data <- read.csv(system.file("extdat", "test-data/harris-iqi-data.csv" ,
                      package = "kraken"), check.names = FALSE)
...
data <- demo_iqi
data$IQI <- 
  c(0.60, 0.61, 0.62, 0.63, 0.64, 0.66, 0.80,0.80, 0.80,
...
```{r seven, message=FALSE}

# Create mock survey dataframe
data <- demo_iqi
data <- data[c(5:6, 14:15, 21:22, 28:29), ]
data$IQI <- c(0.64, 0.66,
              0.64, 0.66,
              0.64, 0.66,
              0.64, 0.66)
...
Failure is detected but ellipse is still drawn around inner most stations
