...
for(i in 1:nrow(model)) {
  ...
  data = transect_hex) +
  geom_star(
  starshape = transect_hex$shape,  size = transect_hex$size, alpha = transect_hex$alpha) +
  ylab("Emmamectin PPM") +
  ggtitle(paste0("Site: ", unique(plot_data$project_id)), 
          subtitle = unique(plot_data$MCFF_Transect)) +
  ...
  geom_vline(xintercept = breach$breachDistance_95thPercentile,
            ...
            linetype = 2,
            linewidth = 1) +
   scale_colour_continuous(palette = c("#FEE0D2", "#FC9272", "#DE2D26"),  na.value = "black") +
  scale_fill_continuous(palette = c("#FEE0D2", "#FC9272", "#DE2D26"), na.value = "black")
  }
  print(p)
}
