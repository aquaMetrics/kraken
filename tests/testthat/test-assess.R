...
test_that("test bearing", {
  ...
  output <- purrr::map_df(
    split(survey_data, survey_data$`Survey ID`),
    function(survey) {
      survey_output <- kraken::consecutive_stations(
        survey,
        use_mean_bearing = FALSE
      )
      survey_output <- survey_output$survey_data
      survey_bearings <- dplyr::select(
        survey_output,
        MCFF,
        Survey_date,
        Transect,
